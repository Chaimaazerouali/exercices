 ...
 * Return: bytes read
 */
ssize_t input_buffer(my_info_t *info, char **buf, size_t *len)
{
    ssize_t r = 0;
    size_t len_p = 0;
 ...
 * Return: bytes read
 */
ssize_t get_my_input(my_info_t *info)
{
    ...
    if (len) /* we have commands left in the chain buffer */
    {
        ...
        check_my_chain(info, buf, &j, i, len);
        while (j < len) /* iterate to semicolon or end */
        {
            if (is_my_chain(info, buf, &j))
                break;
            j++;
        }
    ...
    *buf_p = buf; /* else not a chain, pass back buffer from my_getline() */
    return (r); /* return length of buffer from my_getline() */
}
 ...
 * Return: r
 */
ssize_t read_buffer(my_info_t *info, char *buf, size_t *i)
{
    ssize_t r = 0;
